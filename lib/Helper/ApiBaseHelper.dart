...
class ApiBaseHelper {
  ...
  Future<dynamic> doctorPostAPICall(Uri url, Map parameter) async {
    var responseJson;
    print("parameter : $parameter");
    try {
      final response = await post(url,
          body: parameter.length > 0 ? parameter : null, headers: doctorHeader)
          .timeout(Duration(seconds: timeOut));
  ...
  Future<dynamic> doctorGetAPICall(Uri url) async {
    var responseJson;
    try {
      ...
      print(
          "API = $url,response : ${response.body.toString()}");
      responseJson = _response(response);
    } on SocketException {
      throw FetchDataException('No Internet connection');
    } on TimeoutException {
